...
class ResponseUtils {
    ...
    verifyFieldsArePopulated(fieldsList, data) {
        for (let resource of data) {
            expect(fieldsList.filter((field) => !this.verifyFieldIsPopulated(resource[field])).length).toEqual(2);
        }
        console.log('Response fields are populated');
    }

    verifyFieldsAreTodos(fieldsList, data) {
        for (let resource of data) {
            expect(fieldsList.filter((field) => !this.verifyFieldIsPopulated(resource[field])).length).toEqual(3);
        }
        console.log('Response fields are populated');
    }
